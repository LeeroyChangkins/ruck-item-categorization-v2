#!/usr/bin/env python3
"""
5_upload_to_db.py — v2 DB upload orchestrator.
...
Flow:
  ...
  7. Run pre-upload validation (5_validate.py)
...
ROOT = STEP5_DIR.parent  # v2/ root
STEP4_OUTPUTS = ROOT / "step-4" / "outputs"


def _import_sibling(module_name: str, filename: str):
    """Load a sibling module from this step-5 directory by filename (handles digit-prefixed names)."""
...
def _connect(env: str, user: str, password: str):
    s5 = _import_sibling("s5_db_import", "5_db_import.py")
...
def _do_upload(
    ...
    s5v = _import_sibling("s5_validate", "5_validate.py")
