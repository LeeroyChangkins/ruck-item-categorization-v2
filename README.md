...
| **2.1** | `step-2/2_1_generate_keywords.py` + bigram scripts | Extracts keyword frequencies; builds bigram→category mappings |
| **2.2** | `step-2/2_2_match_items_to_bigrams.py` | Auto-matches items to categories via bigram rules |
| **2.3** | `step-2/2_3_interactive_keyword_match.py` | You manually handle leftover bigram groups |
| **3** | `step-3/3_llm_match_unmatched.py` | LLM matches everything still unmatched |
| **4** | `step-4/4_dedupe_and_summaries.py` | Deduplicates all results across steps 1–3 |
| **5** | `step-5/5_generate_attributes.py` | LLM generates product attributes per leaf category |
